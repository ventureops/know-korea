...
export default function QAEditPage() {
  ...
  useEffect(() => {
    ...
    async function fetchPost() {
      const res = await fetch(`/api/qa?page=1&category=`);
      if (!res.ok) { setLoading(false); return; }
      // We need to fetch the specific post — use supabase client-side or a dedicated endpoint
      // For simplicity, fetch from the list and find
    }
